...
class CriticalRegressionTests(unittest.TestCase):
    ...
    def test_textarea_preserves_existing_answers_when_overwrite_is_disabled(self):
        source = (ROOT / "runAiBot.py").read_text(encoding="utf-8")
        textarea_block = source.split("# Check if it's a textarea question", 1)[1]
        textarea_block = textarea_block.split("# Check if it's a checkbox question", 1)[0]

        overwrite_indent = None
        clear_indent = None
        for line in textarea_block.splitlines():
            if "if not prev_answer or overwrite_previous_answers:" in line:
                overwrite_indent = len(line) - len(line.lstrip(" "))
            if "text_area.clear()" in line:
                clear_indent = len(line) - len(line.lstrip(" "))

        self.assertIsNotNone(overwrite_indent)
        self.assertIsNotNone(clear_indent)
        self.assertGreater(
            clear_indent,
            overwrite_indent,
            "text_area.clear() must stay inside the overwrite check so prefilled cover letters are not wiped",
        )
    ...
    def test_get_job_description_initializes_skip_before_extraction(self):
        source = (ROOT / "runAiBot.py").read_text(encoding="utf-8")
        func = source.split("def get_job_description(", 1)[1]
        func = func.split("def upload_resume(", 1)[0]
        try_pos = func.find("try:")
        skip_pos = func.find("skip = False")
        find_pos = func.find("find_by_class")

        self.assertGreater(skip_pos, 0)
        self.assertGreater(find_pos, 0)
        self.assertLess(skip_pos, try_pos)
        self.assertLess(skip_pos, find_pos)

    def test_gemini_failed_setup_does_not_raise_unbound_local(self):
        ...
        gemini.llm_api_key = ""
        gemini.confirm = MagicMock(return_value="Okay Continue")

        result = gemini.gemini_create_client()

        self.assertIsNone(result)
        gemini.confirm.assert_called_once()
