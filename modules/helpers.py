...
def truncate_for_csv(data, max_length: int = 131000, suffix: str = "...[TRUNCATED]") -> str:
    '''
    Convert values to strings for CSV history writes.

    Python's csv writer does not enforce field size limits; those apply only
    when reading. Truncating here silently drops job descriptions, questions,
    and other history fields, so this helper must not shorten data.
    `max_length` and `suffix` are kept for call-site compatibility.
    '''
    try:
