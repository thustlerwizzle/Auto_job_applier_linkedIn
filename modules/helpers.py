...
def truncate_for_csv(data, max_length: int = 131000, suffix: str = "...[TRUNCATED]") -> str:
    '''
    Convert values for CSV writing. Long fields are preserved because Python's
    CSV writer does not enforce a field-size limit; raise the reader limit with
    `raise_csv_field_size_limit()` instead of dropping job-history data.
