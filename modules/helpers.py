...
def truncate_for_csv(data, max_length: int = 131000, suffix: str = "...[TRUNCATED]") -> str:
    '''
    Convert `data` to a string for CSV writing.

    Python's CSV writer has no field-size limit; truncating here permanently
    drops job-history fields (descriptions, questions, stack traces).
    `max_length` and `suffix` are kept for call-site compatibility and ignored.
    '''
    try:
